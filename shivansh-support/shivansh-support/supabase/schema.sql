...
-- Row Level Security: the site only ever talks to Supabase from server
-- API routes using the service role key, which bypasses RLS. We still
-- enable RLS and add no public policies, so the anon key (if ever leaked)
